# 📊 AI-Staff-HQ Version History

A concise record of milestone releases.

## How to Read This Changelog

This is a personal system that evolves continuously. Version numbers mark major philosophical shifts, not feature releases.

- **Current:** `v2.0.0` — Lean framework optimized for personal use.
- **Previous:** `v1.5.0` — Comprehensive 39-specialist system.
- **Earlier:** `v1.0.x` series — Experimental builds refining structure, templates, and knowledge capture.

## v2.0.0 - The Lean Framework (Oct 2025)
- **Project Refactor:** The project has been significantly refactored to be a lean and extensible framework.
- **Core Team:** The specialist roster has been streamlined to a core team of 5 essential specialists.
- **Bring Your Own Staff:** The project is now designed around the 'bring your own staff' philosophy, with clear instructions and templates for creating new specialists.
- **Streamlined Documentation:** All documentation has been updated to reflect the new lean and extensible approach.

## v1.5.0 – Schema & Autonomy Expansion *(Oct 2025)*
- Migrated the specialist roster to structured YAML and introduced the Actuary for KPI adjudication.
- Launched autonomous workflow category (e.g., **Autonomous Weekly Content Pipeline**) and formalized the `/logs` memory system.
- Added `/tools` and `/workflows/automations` to host executable assets from the Technical Department.

## v1.4.0 – Knowledge Infrastructure *(Jul 2025)*
- Expanded handbooks and templates to cover end-to-end project orchestration.
- Added logging protocols and retrospective templates to build a learning feedback loop.
- Delivered additional automation concepts (trend tracking, news summarizer) to showcase technical capability.

## v1.3.0 – Workforce Maturity *(Apr 2025)*
- Rounded out the multi-department staff directory with expanded Creative, Strategy, and Specialized expertise.
- Introduced project templates (brief, onboarding, retrospective) to standardize execution quality.
- Documented the “Happy Path” launch playbook for cross-department collaboration.

## v1.2.0 – Foundation Build *(Jan 2025)*
- Established repository structure, staff persona templates, and knowledge-base scaffolding.
- Delivered initial workflows and integration notes to guide early adoption.
- Set quality standards and collaboration patterns for the founding 17-specialist cohort.
